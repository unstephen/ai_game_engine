...
RHIResult D3D12Device::Initialize(const DeviceDesc& desc)
{
    ...
    for (UINT adapterIndex = 0; m_dxgiFactory->EnumAdapters1(adapterIndex, &adapter) != DXGI_ERROR_NOT_FOUND;
         ++adapterIndex)
    {
    ...
    if (SUCCEEDED(selectedAdapter->QueryInterface(IID_PPV_ARGS(&adapter3))))
    {
        ...
        {
            RHI_LOG_DEBUG("D3D12Device: 当前显存使用: %llu MB", localMemoryInfo.CurrentUsage / (1024 * 1024));
        }
    }
...
std::unique_ptr<ITexture> D3D12Device::CreateTexture(const TextureDesc& desc)
{
    ...
    RHI_LOG_INFO("D3D12Device: 纹理创建成功 (%ux%u, 格式=%d)", desc.width, desc.height, static_cast<int>(desc.format));
    return texture;
}
                                                         ...
                                                         Format format)
{
    ...
    RHI_LOG_INFO("D3D12Device: 交换链创建成功 (%ux%u, 格式=%d)", width, height, static_cast<int>(format));
    return swapChain;
}

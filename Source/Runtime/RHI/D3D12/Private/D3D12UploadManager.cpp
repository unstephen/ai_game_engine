...
D3D12UploadManager::D3D12UploadManager(D3D12Device* device)
    : m_device(device)
{
...
UploadAllocation D3D12UploadManager::AllocateEx(uint64_t size, uint64_t alignment) {
    ...
    if (alignedOffset + alignedSize > block.capacity) {
        RHI_LOG_ERROR("D3D12UploadManager: 块空间不足（需要: %llu, 可用: %llu）",
            alignedOffset + alignedSize, block.capacity);
        return result;
    }
    ...
    if (block.used >= block.capacity * 0.95) {
        block.isFull = true;
        RHI_LOG_DEBUG("D3D12UploadManager: 块 %u 已满 (%.1f%%)",
            blockIndex, block.used * 100.0 / block.capacity);
    }
...
void D3D12UploadManager::CleanupCompletedUploads(uint64_t completedFenceValue) {
    ...
    for (auto& block : m_blocks) {
        if (block.fenceValue > 0 && block.fenceValue <= completedFenceValue) {
            block.Reset();
            RHI_LOG_DEBUG("D3D12UploadManager: 块 %u 已重置（围栏值: %llu <= %llu）",
                block.blockIndex, block.fenceValue, completedFenceValue);
        }
    }
    
    if (cleanedCount > 0 || m_config.enableDebugLog) {
        RHI_LOG_DEBUG("D3D12UploadManager: 清理完成（围栏值: %llu，清理数: %zu）",
            completedFenceValue, cleanedCount);
    }
}

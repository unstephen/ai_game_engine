...
RHIResult D3D12SwapChain::Initialize(D3D12Device* device, const SwapChainDesc& desc)
{
    ...
    if (desc.width == 0 || desc.height == 0) {
        RHI_LOG_ERROR("D3D12SwapChain::Initialize: 无效的尺寸 (%ux%u)", 
            desc.width, desc.height);
        return RHIResult::InvalidParameter;
    }
    
    if (desc.bufferCount < 2 || desc.bufferCount > 4) {
        RHI_LOG_ERROR("D3D12SwapChain::Initialize: 无效的缓冲区数量 %u (应为 2-4)", 
            desc.bufferCount);
        return RHIResult::InvalidParameter;
    }
    ...
    RHI_LOG_INFO("D3D12SwapChain 初始化成功: %ux%u, %u 缓冲区, 格式 %d",
...
void D3D12SwapChain::Resize(uint32_t width, uint32_t height)
{
    ...
    RHI_LOG_INFO("D3D12SwapChain::Resize: %ux%u -> %ux%u", 
...
ITexture* D3D12SwapChain::GetBackBuffer(uint32_t index)
{
    if (index >= m_bufferCount) {
        RHI_LOG_ERROR("D3D12SwapChain::GetBackBuffer: 索引越界 %u (缓冲区数量: %u)",
            index, m_bufferCount);
        return nullptr;
    }
...
RHIResult D3D12SwapChain::CreateRenderTargetViews(D3D12Device* device)
{
    ...
    // 获取描述符增量大小
    m_rtvDescriptorSize = d3dDevice->GetDescriptorHandleIncrementSize(
    ...
    for (UINT i = 0; i < m_bufferCount; i++) {
        ...
        if (FAILED(hr)) {
            RHI_LOG_ERROR("D3D12SwapChain::CreateRenderTargetViews: 获取后备缓冲区 %u 失败: 0x%08X", 
                i, hr);
            return RHIResult::ResourceCreationFailed;
        }
        ...
        if (!m_debugName.empty()) {
            wchar_t name[64];
            swprintf_s(name, L"%s_BackBuffer%u", 
                std::wstring(m_debugName.begin(), m_debugName.end()).c_str(), i);

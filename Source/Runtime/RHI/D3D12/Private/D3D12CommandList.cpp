...
D3D12CommandList::D3D12CommandList() : m_type(D3D12_COMMAND_LIST_TYPE_DIRECT)
{
    // 初始化描述符堆数组
...
RHIResult D3D12CommandList::Initialize(D3D12Device* device, ID3D12CommandAllocator* allocator,
                                       D3D12_COMMAND_LIST_TYPE type)
{
    ...
    HRESULT       hr        = d3dDevice->CreateCommandList(0,         // nodeMask（单 GPU 为 0）
                                              type,      // 命令列表类型
                                              allocator, // 命令分配器
                                              nullptr,   // 初始管线状态
...
void D3D12CommandList::ClearDepthStencil(ITexture* depthStencil, bool clearDepth, bool clearStencil, float depth,
                                         uint8_t stencil)
{
    if (!CheckRecordingState())
        return;
...
void D3D12CommandList::SetRenderTargets(ITexture* const* renderTargets, uint32_t count, ITexture* depthStencil)
{
    if (!CheckRecordingState())
        return;
...
void D3D12CommandList::DrawIndexedInstanced(uint32_t indexCount, uint32_t instanceCount, uint32_t startIndex,
...
void D3D12CommandList::SetGraphicsRootConstantBufferView(uint32_t rootParameterIndex, uint64_t address)
{
    if (!CheckRecordingState())
        return;
...
void D3D12CommandList::SetGraphicsRootShaderResourceView(uint32_t rootParameterIndex, uint64_t address)
{
    if (!CheckRecordingState())
        return;
...
void D3D12CommandList::SetGraphicsRootUnorderedAccessView(uint32_t rootParameterIndex, uint64_t address)
{
    if (!CheckRecordingState())
        return;
...
void D3D12CommandList::SetComputeRootConstantBufferView(uint32_t rootParameterIndex, uint64_t address)
{
    if (!CheckRecordingState())
        return;
...
void D3D12CommandList::SetDescriptorHeaps(ID3D12DescriptorHeap* const* heaps, uint32_t count)
{
    if (!CheckRecordingState())
        return;
...
void D3D12CommandList::CopyBufferRegion(ID3D12Resource* dest, uint64_t destOffset, ID3D12Resource* src,
...
void D3D12CommandList::CopyTextureRegion(ID3D12Resource* dest, uint32_t destX, uint32_t destY, uint32_t destZ,
                                         ...
                                         uint32_t width, uint32_t height, uint32_t depth)
{
    ...
    m_commandList->CopyTextureRegion(&destLocation, destX, destY, destZ, &srcLocation, &srcBox);
}
...
void D3D12CommandList::TransitionBarrier(ID3D12Resource* resource, D3D12_RESOURCE_STATES before,
...
void D3D12CommandList::ResourceBarriers(const D3D12_RESOURCE_BARRIER* barriers, uint32_t count)
{
    if (!CheckRecordingState())
        return;

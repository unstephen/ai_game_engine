...
{
    RHI_LOG_INFO("D3D12Buffer: 开始初始化 (大小=%llu, 用途=0x%X)", desc.size, static_cast<uint32_t>(desc.usage));
...
void D3D12Buffer::UpdateData(const void* data, uint64_t size, uint64_t offset)
{
    ...
    {
        RHI_LOG_ERROR("D3D12Buffer: 更新数据超出缓冲区范围 (offset=%llu, size=%llu, bufferSize=%llu)", offset, size,
                      m_size);
        return;
    }
...
RHIResult D3D12Buffer::CreateResource(D3D12Device* device, const BufferDesc& desc)
{
    ...
    D3D12_HEAP_TYPE heapType = m_cpuAccessible ? D3D12_HEAP_TYPE_UPLOAD : D3D12_HEAP_TYPE_DEFAULT;

    RHI_LOG_DEBUG("D3D12Buffer: 堆类型=%s", m_cpuAccessible ? "UPLOAD" : "DEFAULT");
    ...
    D3D12_RESOURCE_DESC resourceDesc = CreateBufferDesc(bufferSize, resourceFlags);

    // 结构化缓冲区
    if (desc.stride > 0 && (desc.usage & BufferUsage::ShaderResource) != BufferUsage::None)
    {
    ...
    HRESULT hr = d3dDevice->CreateCommittedResource(&heapProps, D3D12_HEAP_FLAG_NONE, &resourceDesc, m_resourceState,
                                                    nullptr, // 清除值（缓冲区不需要）
...
D3D12_RESOURCE_STATES D3D12Buffer::DetermineResourceState(BufferUsage usage, ResourceState initialState)
{

    // 如果指定了初始状态，转换为 D3D12 状态
    if (initialState != ResourceState::Undefined && initialState != ResourceState::Common)
    {

...
RHIResult D3D12Texture::Initialize(D3D12Device* device, const TextureDesc& desc) {
    ...
    RHI_LOG_INFO("D3D12Texture: 纹理创建成功 (%ux%u, 格式=%d, Mips=%u, 数组=%u)",
        m_width, m_height, static_cast<int>(m_format), m_mipLevels, m_arraySize);
...
RHIResult D3D12Texture::CreateSRV(
    D3D12Device* device,
...
RHIResult D3D12Texture::CreateRTV(
    D3D12Device* device,
    D3D12_CPU_DESCRIPTOR_HANDLE handle,
    uint32_t mipSlice,
    uint32_t arraySlice) {
...
RHIResult D3D12Texture::CreateDSV(
    D3D12Device* device,
    D3D12_CPU_DESCRIPTOR_HANDLE handle,
    uint32_t mipSlice) {
...
RHIResult D3D12Texture::CreateUAV(
    D3D12Device* device,
    D3D12_CPU_DESCRIPTOR_HANDLE handle,
    uint32_t mipSlice) {
...
RHIResult D3D12Texture::UploadData(
    D3D12Device* device,
    ...
    uint64_t uploadBufferSize = GetRequiredIntermediateSize(
        m_resource.Get(),
        CalcSubresource(mipLevel, arraySlice),
        1);
...
D3D12_RESOURCE_STATES D3D12Texture::DetermineResourceState(
    TextureUsage usage,

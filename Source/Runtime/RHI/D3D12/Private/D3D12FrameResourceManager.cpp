...
D3D12FrameResourceManager::D3D12FrameResourceManager(D3D12Device* device, uint32_t numFrames)
    : m_device(device), m_numFrames(numFrames), m_currentFrameIndex(0), m_nextFenceValue(1)
{
...
RHIResult D3D12FrameResourceManager::Initialize()
{
    ...
    for (uint32_t i = 0; i < m_numFrames; ++i)
    {
        ...
        hr = d3dDevice->CreateFence(0,                     // 初始值
        ...
        ctx.fenceEvent = CreateEvent(nullptr, // 默认安全属性
                                     FALSE,   // 自动重置
                                     FALSE,   // 初始状态：无信号
                                     nullptr  // 无名称
        );
        ...
        if (!ctx.fenceEvent)
        {
            RHI_LOG_ERROR("D3D12FrameResourceManager: 创建围栏事件失败 (帧 %u, 错误: %u)", i, GetLastError());
            Shutdown();
        ...
        hr = d3dDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, // 直接命令列表类型
                                               IID_PPV_ARGS(&ctx.commandAllocator));

        if (FAILED(hr))
        {
            RHI_LOG_ERROR("D3D12FrameResourceManager: 创建命令分配器失败 (帧 %u, HRESULT: 0x%08X)", i, hr);
            Shutdown();
            return RHIResult::InternalError;
        }
...
void D3D12FrameResourceManager::BeginFrame()
{
    ...
    if (ctx.fenceValue > 0)
    {
        RHI_LOG_DEBUG("D3D12FrameResourceManager: 等待 GPU 完成帧 %u (围栏值: %llu)", m_currentFrameIndex,
                      ctx.fenceValue);
...
void D3D12FrameResourceManager::EndFrame()
{
    ...
    RHI_LOG_DEBUG("D3D12FrameResourceManager: 帧 %u 围栏值更新为 %llu", m_currentFrameIndex, ctx.fenceValue);
...
void D3D12FrameResourceManager::WaitForFrame(uint32_t frameIndex)
{
    ...
    {
        RHI_LOG_ERROR("D3D12FrameResourceManager: 无效的帧索引 %u (范围: 0-%u)", frameIndex, m_numFrames - 1);
        return;
    }
...
void D3D12FrameResourceManager::WaitForFence(ID3D12Fence* fence, uint64_t fenceValue, HANDLE fenceEvent)
{
    if (!fence || !fenceEvent)
    {
    ...
    {
        RHI_LOG_DEBUG("D3D12FrameResourceManager: 围栏已完成 (值: %llu >= %llu)", completedValue, fenceValue);
        return;
    }

    RHI_LOG_DEBUG("D3D12FrameResourceManager: 等待围栏 (当前: %llu, 目标: %llu)", completedValue, fenceValue);
    ...
    switch (waitResult)
    {
        ...
        case WAIT_FAILED:
            RHI_LOG_ERROR("D3D12FrameResourceManager: WaitForSingleObject 失败 (错误: %u)", GetLastError());
            break;

        default:
            RHI_LOG_WARNING("D3D12FrameResourceManager: WaitForSingleObject 意外返回值: %u", waitResult);
            break;
    }
}
...
void D3D12FrameResourceManager::CleanupFrameResources(D3D12FrameContext& ctx)
{
    ...
    {
        RHI_LOG_DEBUG("D3D12FrameResourceManager: 帧 %u 清理完成 (上传: %zu, 临时: %zu, 删除: %zu)", ctx.frameIndex,
                      uploadCount, tempCount, deleteCount);
    }
}

...
struct UploadBlock
{
    ...
    /// 获取可用空间
    uint64_t GetAvailableSpace() const { return capacity - used; }
...
class D3D12UploadManager : public IUploadManager
{
  public:
     ...
     * @return RHIResult::OutOfMemory 内存不足
     */
    RHIResult UploadBufferData(ID3D12GraphicsCommandList* commandList, ID3D12Resource* destBuffer, uint64_t destOffset,
     ...
     * @return RHIResult::OutOfMemory 内存不足
     */
    RHIResult UploadTextureData(ID3D12GraphicsCommandList* commandList, ID3D12Resource* destTexture, const void* data,
                                uint64_t dataSize, const D3D12_PLACED_SUBRESOURCE_FOOTPRINT& footprint,
  ...
  private:
     ...
     * @return 对齐后的大小
     */
    static uint64_t AlignSize(uint64_t size, uint64_t alignment) { return (size + alignment - 1) & ~(alignment - 1); }

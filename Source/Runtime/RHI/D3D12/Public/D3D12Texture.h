...
class D3D12Texture : public ITexture
{
  public:
     ...
     * @return RHIResult 结果码
     */
    RHIResult CreateSRV(D3D12Device* device, D3D12_CPU_DESCRIPTOR_HANDLE handle, uint32_t mostDetailedMip = 0,
                        int32_t mipLevels = -1);
     ...
     * @return RHIResult 结果码
     */
    RHIResult CreateRTV(D3D12Device* device, D3D12_CPU_DESCRIPTOR_HANDLE handle, uint32_t mipSlice = 0,
     ...
     * @return RHIResult 结果码
     */
    RHIResult CreateDSV(D3D12Device* device, D3D12_CPU_DESCRIPTOR_HANDLE handle, uint32_t mipSlice = 0);
     ...
     * @return RHIResult 结果码
     */
    RHIResult CreateUAV(D3D12Device* device, D3D12_CPU_DESCRIPTOR_HANDLE handle, uint32_t mipSlice = 0);
     ...
     * @return RHIResult 结果码
     */
    RHIResult UploadData(D3D12Device* device, ID3D12GraphicsCommandList* commandList, const void* data, size_t dataSize,
                         uint32_t rowPitch, uint32_t arraySlice = 0, uint32_t mipLevel = 0);
  ...
  private:
     ...
     */
    static D3D12_RESOURCE_STATES DetermineResourceState(TextureUsage usage, ResourceState initialState);

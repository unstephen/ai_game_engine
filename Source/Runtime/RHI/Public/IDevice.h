...
class RHI_API IDevice
{
  public:
    ...
    /// 创建根签名
    virtual std::unique_ptr<IRootSignature> CreateRootSignature(const RootSignatureDesc& desc) = 0;

    /// 创建图形管线
    virtual std::unique_ptr<IPipelineState> CreateGraphicsPipeline(const GraphicsPipelineDesc& desc) = 0;

    /// 创建计算管线
    virtual std::unique_ptr<IPipelineState> CreateComputePipeline(const ComputePipelineDesc& desc) = 0;
